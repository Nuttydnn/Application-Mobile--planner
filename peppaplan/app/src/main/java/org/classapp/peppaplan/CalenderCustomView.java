...
public class CalenderCustomView extends LinearLayout {
    ...
    public CalenderCustomView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        IntializeUILayout();
        SetupCalendar();
        ImageButton button = findViewById(R.id.diary);
                ...
                AddEvent.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if(alarmMe.isChecked()){
                            SaveEvent(EventBody.getText().toString(), EventTime.getText().toString(), date, monthFormat.format(dateList.get(position)), yearFormat.format(dateList.get(position)),"on");
                            SetupCalendar();
                            Calendar calendar = Calendar.getInstance();
                            calendar.set(alarmYear,alarmMonth,alarmDay,alarmHour,alarmMinuit);
                            setAlarm(calendar,EventBody.getText().toString(),EventTime.getText().toString(),getRequestCode(date,EventBody.getText().toString(),EventTime.getText().toString()));
                            alertDialog.dismiss();

                        }else{
                            SaveEvent(EventBody.getText().toString(), EventTime.getText().toString(), date, monthFormat.format(dateList.get(position)), yearFormat.format(dateList.get(position)),"off");
                            SetupCalendar();
                            alertDialog.dismiss();
      ...
      gridView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
        ...
        public CalenderCustomView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
// Toast.makeText(context, String.valueOf(arrayList.size()), Toast.LENGTH_SHORT).show();
    ...
    private  void  OpenActivity(){
        Intent intent = new Intent(context, NoteMe.class);
